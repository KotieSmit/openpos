...
<?php echo form_open("touchSales/change_mode",array('id'=>'mode_form')); ?>
	<span><?php echo $this->lang->line('sales_mode') ?></span>
<?php echo form_dropdown('mode',$modes,$mode,'onchange="$(\'#mode_form\').submit();"'); ?>
<div id="show_suspended_sales_button">
	<?php echo anchor("touchSales/suspended/width:425",
...
<?php echo form_open("touchSales/add",array('id'=>'add_item_form')); ?>
...
else
{
	foreach(array_reverse($cart, true) as $line=>$item)
	{
		...
		echo form_open("touchSales/edit_item/$line");
	?>
...
<div id="overall_sale">
	...
	if(isset($customer))
	{
		...
		echo anchor("touchSales/remove_customer",'['.$this->lang->line('common_remove').' '.$this->lang->line('customers_customer').']');
	}
	else
	{
		echo form_open("touchSales/select_customer",array('id'=>'select_customer_form')); ?>
	...
	?>
    	...
    	<div id="Cancel_sale">
		<?php echo form_open("touchSales/cancel_sale",array('id'=>'cancel_sale_form')); ?>
		...
		?>
			<div id="finish_sale">
				<?php echo form_open("touchSales/complete",array('id'=>'finish_sale_form')); ?>
	...
	<div id="Payment_Types" >
		...
		<div style="height:100px;">
			...
			<?php echo form_open("touchSales/add_payment",array('id'=>'add_payment_form')); ?>
			<table width="100%">
			<tr>
			<td>
		...
		?>
			...
			<?php
				...
				{
				echo form_open("touchSales/edit_payment/$payment_id",array('id'=>'edit_payment_form'.$payment_id));
				?>
	            <tr>
	            <td><?php echo anchor( "touchSales/delete_payment/$payment_id", '['.$this->lang->line('common_delete').']' ); ?></td>
...
{
    $("#item").autocomplete('<?php echo site_url("touchSales/item_search"); ?>',
    {
    	minChars:0,
    	max:100,
    	selectFirst: false,
    ...
    $("#customer").autocomplete('<?php echo site_url("touchSales/customer_search"); ?>',
    {
    	minChars:0,
    	delay:10,
    	max:100,
	...
	{
		$.post('<?php echo site_url("touchSales/set_comment");?>', {comment: $('#comment').val()});
	});
	
	$('#email_receipt').change(function() 
	{
		$.post('<?php echo site_url("touchSales/set_email_receipt");?>', {email_receipt: $('#email_receipt').is(':checked') ? '1' : '0'});
	});
	...
	$("#suspend_sale_button").click(function()
	{
    	...
    	{
			$('#finish_sale_form').attr('action', '<?php echo site_url("touchSales/suspend"); ?>');
    		$('#finish_sale_form').submit();
    	}
	});
